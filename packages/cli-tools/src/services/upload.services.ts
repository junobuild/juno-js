...
const batchUploadFiles = async ({
  ...
  const uploadFiles = async (groupFiles: FileAndPaths[]) => {
    ...
    for (let i = 0; i < groupFiles.length; i += UPLOAD_BATCH_SIZE) {
      ...
      const buildTitle = ({file, paths}: FileAndPaths): string => {
        const title = `Uploading ${relative(sourceAbsolutePath, file.file)}`;

        try {
          const size = fileSizeInBytes(paths.filePath);
          return `${title} (${formatBytes(size)})`;
        } catch (_err: unknown) {
          // We ignore potential lstat errors to rather bubble the errors generated by the upload function.
          // Assuming that if lstat fails, the upload will fail as well, the source file being invalid.
          return title;
        }
      };
