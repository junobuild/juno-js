...
function publish_npm() {
  ...
  if [ "$LOCAL_SHASUM" == "$NPM_SHASUM" ]; then
    ...
    npm publish --prefix dist --workspace=packages/"$lib" --provenance --access public
  fi
}

# Tips: libs use by other libs first
LIBS=utils,config,config-loader,cli-tools,storage,core,admin,console,analytics,core-peer
