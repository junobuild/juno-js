...
describe('json-utils', () => {
  describe('stringify', () => {
    ...
    it('should stringify Principal with a custom representation', () => {
      ...
      expect(JSON.stringify(principal, jsonReplacer)).toEqual('{"__principal__":"2vxsx-fae"}');
      ...
      expect(JSON.stringify({principal: rootCanisterId}, jsonReplacer)).toEqual(
  ...
  describe('parse', () => {
    ...
    it('should parse principal from a custom representation', () => {
      ...
      expect(JSON.parse('{"__principal__":"2vxsx-fae"}', jsonReviver)).toEqual(principal);
      expect(JSON.parse('{"principal":{"__principal__":"2vxsx-fae"}}', jsonReviver)).toEqual({
        principal
      });

      const rootCanisterId = Principal.fromText('tmxop-wyaaa-aaaaa-aaapa-cai');

      expect(JSON.parse('{"__principal__":"tmxop-wyaaa-aaaaa-aaapa-cai"}', jsonReviver)).toEqual(
        rootCanisterId
      );
      expect(
        JSON.parse('{"principal":{"__principal__":"tmxop-wyaaa-aaaaa-aaapa-cai"}}', jsonReviver)
      ).toEqual({principal: rootCanisterId});
    });

    it('should parse principal to object', () => {
      const obj = JSON.parse('{"__principal__":"tmxop-wyaaa-aaaaa-aaapa-cai"}', jsonReviver);
      expect(obj instanceof Principal).toBeTruthy();
      expect((obj as Principal).toText()).toEqual('tmxop-wyaaa-aaaaa-aaapa-cai');
    });
